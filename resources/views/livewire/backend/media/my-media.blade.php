<x-backend-layout>
<div>
    @section('title','Show All My Media')
    @push('page-css')
    <style>
        *[tooltip]:focus:after {
        ...
        select:disabled:not([read-only="false"]) {
            background-color: rgb(248, 248, 248);
            ;
        }
                ...
                <div class="col-12">
                    <div class="card p-1">
                        <div class="card-header border-bottom">
                            ...
                            <a class="btn btn-outline-danger" href="{{ route('admin-panel.media') }}"><i
                                    class="fa fa-th-large me-1"></i> Grid View</a>
                            @endcanany
                            @can('isAuthor')
                            <a class="btn btn-outline-danger" href="{{ route('author.media') }}"><i
                                    class="fa fa-th-large me-1"></i> Grid View</a>
                            @endcan

                            <h1 class="card-title" style="font-size: 28px">Display All Media</h1>
                            <button class="btn btn-primary btn-lg" wire:click.prevent="cancel()" data-bs-toggle="modal"
                                data-id="1" data-bs-target="#addMediaModal">Add New Media</button>
                        ...
                        <div class="card-datatable table-responsive">
                            ...
                            @endif
                            <table wire:ignore id="postsTable" class="table table-bordered table-hover dt-responsive"
                                style="width:100%">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="#" data-bs-toggle="modal" data-bs-target="#updateMediaModal"
                                                wire:click="details({{ $media->id }})">{{ $media->title }}</a>
                                        </td>
                                        <td><a href="#" data-bs-toggle="modal" data-bs-target="#updateMediaModal"
                                                wire:click="details({{ $media->id }})">{{ $media->slug }}</a></td>
                                        <td class="text-center">
                                            <a href="#" data-bs-toggle="modal" data-bs-target="#updateMediaModal"
                                                wire:click="details({{ $media->id }})"><img
                                                    src="{{ asset('storage/media/'.$media->media_name) }}" width="40"
                                                    height="40" alt="{{ $media->title }}"></a>
                                        </td>
                                        <td>
                                            <div class="input-group">
                                                <input type="text" class="form-control" id="{{ $media->slug }}"
                                                    value="{{ url('storage/media/'.$media->media_name) }}" readonly>
                                                <span class="input-group-text"><a href="javascript:void(0)"
                                                        class="copy-btn" tooltip="Copied"
                                        ...
                                        <td class="text-center">
                                            ...
                                            <a href="#" data-bs-toggle="modal" data-bs-target="#updateMediaModal"
                                                wire:click="edit({{ $media->id }})" class="item-edit">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                    viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                    stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
